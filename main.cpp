...
public:
	...
	void encodeMethod1(ifstream& in, ofstream& encode) { // encode zeros and no wrap around
	...
	void encodeMethod4(ifstream& in, ofstream& encode) {


		if(!in.eof()) {
			string numRows, numCols, minVal, maxVal, curVal, nextVal;
			in >> numRows; this->numRows = stoi(numRows);
			in >> numCols; this->numCols = stoi(numCols);
			in >> minVal; this->minVal = stoi(minVal);
			in >> maxVal; this->maxVal = stoi(maxVal);
			encode << numRows << " " << numCols  << " " << minVal << " " << maxVal << endl;
...
int main(int argc, char* argv[]) {
	...
	cout << nameEncodeFile << endl;
	cout << endl << nameEncodeFile;
	//nameEncodeFile.append(".txt");
	//ofstream encodeFile; encodeFile.open(nameEncodeFile.c_str());
	//encodeFile << numRows << " " << numCols  << " " << minVal << " " << maxVal << endl;
	//encodeFile << nameEncodeFile << "Method"<< whichMethod << endl;
